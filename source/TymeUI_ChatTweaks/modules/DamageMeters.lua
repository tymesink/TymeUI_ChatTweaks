...
local moduleName = 'DamageMeters';
local displayName = 'Damage Meters';
local _G = getfenv(0)
-----------------------------------------------------------------------
-- internal libraries
-----------------------------------------------------------------------
	...
	for k, v in ipairs(firstLines) do
		...
		if message:match(v) then
			...
			for i, j in ipairs(meters) do
				...
				if j.source == source and j.event == event and elapsed < db.capture then
					newID = i
					return true, true, format("ChatTweaks: |HECT:%1$d|h|cFFFFFF00[%2$s]|r|h", newID or 0, message or "nil")
				end
			end
	...
	for _, event in pairs(events) do
		...
		if not options[name] then
			options[name] = {
				...
				order = 13 + i,
				name = setting .. " Chat",
				desc = 'What to do with Recount/Skada/TinyDPS reports in this channel.',
				values = {
					[1] = 'Do Nothing',
					[2] = 'Compress',
					[3] = 'Suppress'
...
function Module:Info()
	return 'Suppress Recount/Skada/TinyDPS output into a clickable link, or filter it entirely.'
end
	...
	if not options.capture then
		options = {
			capture = {
				...
				order = 100,
				name = 'Capture Delay',
				desc = 'Time, in seconds, the module will wait after the first line is found to assume it is complete.\n\n|cffffff00One (1) second seems to work.|r',
...
function Module:ParseLink(link, text, button, frame)
	...
	if linkType == "ECT" then
		...
		ItemRefTooltip:AddLine(format('Reported by %s', meters[meterID].source))
